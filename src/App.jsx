...
import { ChevronDown, Zap, Copy, ExternalLink, TrendingUp, TrendingDown, Wallet, CheckCircle, XCircle } from 'lucide-react';
...
const ContractInfoCard = ({ contractAddress, aiAgent, isOwner, onUpdateProbability, onSetAIAgent, isContractReady }) => {
  ...
  const handleUpdateProbability = async () => {
    ...
    setSuccessMsg(null);
    try {
      const result = await onUpdateProbability(selectedTeam, probability);
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <div className="space-y-4">
          ...
          <div>
            ...
            {/* Set AI Agent Button - Added here */}
            <div className="mt-3">
              {!isAgentSet ? (
                <button
                  onClick={handleSetAIAgent}
                  disabled={isSettingAgent}
                  className="w-full mt-2 px-3 py-2 rounded-lg bg-gradient-to-r from-emerald-500 to-teal-500 text-white text-xs font-medium disabled:opacity-50 hover:shadow-lg transition-all flex items-center justify-center gap-2"
                >
                  {isSettingAgent ? (
                    <>
          ...
          <div>
            ...
            <div className="relative">
              <button
                ...
                className="w-full flex items-center justify-between p-3 bg-gradient-to-r from-black/50 to-black/30 rounded-xl border border-white/10 hover:border-cyan-500/30 transition-all"
              >
                {selectedTeamData ? (
              ...
              <div className="mt-4 p-4 bg-black/30 rounded-xl border border-white/10">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => setProbability(selectedTeamData.currentProb)}
                ...
                <input 
                  ...
                  style={{
                    background: `linear-gradient(90deg, #06b6d4 0%, #a855f7 ${probability}%, rgba(255,255,255,0.1) ${probability}%)`
                  }}
                />
                ...
                <div className="flex items-center gap-2 mt-4 p-2 bg-cyan-500/5 rounded-lg border border-cyan-500/20">
                  ...
                  <span className="text-xs text-cyan-400/70">
                    {probability > selectedTeamData.currentProb 
                      ? `📈 Increasing from ${selectedTeamData.currentProb}% to ${probability}%`
                      : probability < selectedTeamData.currentProb
                      ? `📉 Decreasing from ${selectedTeamData.currentProb}% to ${probability}%`
                      : `⚡ No change from current ${selectedTeamData.currentProb}%`
                    }
                  </span>
                </div>
              </div>
            ...
            <button 
              ...
              disabled={isUpdating || !selectedTeam || !isAgentSet}
              className="w-full mt-4 px-4 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-purple-500 text-white text-sm font-medium disabled:opacity-50 hover:shadow-lg hover:shadow-cyan-500/25 transition-all flex items-center justify-center gap-2"
            >
              {isUpdating ? (
                <>
            ...
            {errorMsg && (
              <div className="mt-3 p-2 bg-red-500/20 border border-red-500/50 rounded-lg text-red-400 text-xs text-center">
                {errorMsg}
              </div>
            )}
            {successMsg && (
              <div className="mt-3 p-2 bg-emerald-500/20 border border-emerald-500/50 rounded-lg text-emerald-400 text-xs text-center">
                {successMsg}
              </div>
            )}
            
            <p className="text-xs text-emerald-400/70 text-center mt-3">
              {isAgentSet ? '✅ AI Agent set. Ready to update probabilities!' : '⚠️ Please set AI Agent first to enable updates'}
            </p>
          </div>
        </div>
      </div>
...
// Agent Terminal Component (Real Transactions)
      ...
      <div className="border-b border-white/10 px-6 py-4">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-white font-bold">AI Agent Console</h3>
              <p className="text-xs text-white/40">Autonomous portfolio manager on X Layer</p>
            </div>
          </div>
          <button
            onClick={runAgentCycle}
            disabled={isRunning || !isContractReady || !isAgentSet}
            className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-purple-500 text-white font-medium hover:shadow-lg transition-all disabled:opacity-50"
          >
      ...
      {selectedMatch && (
        <div className="px-6 py-3 bg-yellow-500/10 border-b border-yellow-500/20">
          <div className="flex items-center justify-between">
            <span className="text-yellow-400 text-sm">Currently Analyzing:</span>
            <span className="text-white font-medium">{selectedMatch.teamA} vs {selectedMatch.teamB}</span>
          </div>
        </div>
      )}
...
const AppContent = () => {
  ...
  const isContractReady = true; // Contract is deployed and ready
  const isAgentSet = aiAgent && aiAgent !== "0x0000000000000000000000000000000000000000";
      ...
      <div className="min-h-screen bg-black flex items-center justify-center p-4">
        <div className="text-center max-w-md">
          ...
          <button 
            ...
            className="px-8 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-purple-500 text-white font-semibold hover:shadow-lg transition-all disabled:opacity-50 w-full flex items-center justify-center gap-2"
          >
          ...
          <div className="mx-auto max-w-7xl space-y-8">
            {txStatus && (
              <div className={`p-4 rounded-xl backdrop-blur-sm ${
                txStatus.type === 'success' ? 'bg-emerald-500/20 border border-emerald-500/50 text-emerald-400' :
                txStatus.type === 'error' ? 'bg-red-500/20 border border-red-500/50 text-red-400' :
                'bg-yellow-500/20 border border-yellow-500/50 text-yellow-400'
              }`}>
                <div className="flex items-center gap-2">
                  {txStatus.type === 'pending' && <div className="w-4 h-4 border-2 border-yellow-400/30 border-t-yellow-400 rounded-full animate-spin" />}
                  {txStatus.message}
                </div>
              </div>
            )}
